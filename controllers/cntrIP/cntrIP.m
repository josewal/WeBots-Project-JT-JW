...
pathPID = PID(0.004, 0.1, 0.04);
pathPID.enable()
pathPID.setLimits(-0.1,0.1)
%======================================================%
...
while wb_robot_step(TIME_STEP) ~= -1
    ...
    [path_offset, mid_offset, endpoint_offset, path_angle, gotLine] = lineRecognition(input_image);
    ...
    pathPID.setSetpoint(path_offset + mid_offset);
    pathPID.update(0);
    
    if gotLine
      desired_yaw = desired_yaw + pathPID.output;
      desired_velocity = 2 * (1 - abs(endpoint_offset)/32);
    end
